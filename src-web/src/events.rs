//! Shared event names used by the Web runtime.
//!
//! Keep these names aligned with `src-tauri/src/events.rs`. The Web runtime
//! transports the same event contract over WebSocket.

pub const MAIL_SYNC_PROGRESS: &str = "mail:sync-progress";
pub const MAIL_SYNC_COMPLETE: &str = "mail:sync-complete";
pub const MAIL_REALTIME_STATUS: &str = "mail:realtime-status";
pub const MAIL_FOLDER_CHANGED: &str = "mail:folder-changed";
pub const MAIL_ERROR: &str = "mail:error";
pub const MAIL_NEW: &str = "mail:new";
pub const MAIL_NOTIFICATION_OPEN: &str = "mail:notification-open";
pub const MAIL_UNSNOOZED: &str = "mail:unsnoozed";
pub const MAIL_PENDING_OPS_CHANGED: &str = "mail:pending-ops-changed";
pub const APP_OPEN_MAILTO: &str = "app:open-mailto";
