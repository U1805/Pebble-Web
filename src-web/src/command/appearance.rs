...
mod tests {
    ...
    #[test]
    fn detects_supported_background_image_formats() {
        assert_eq!(detect_extension(b"\x89PNG\r\n\x1a\nrest").unwrap(), "png");
        assert_eq!(detect_extension(b"\xff\xd8\xffrest").unwrap(), "jpg");
        assert_eq!(detect_extension(b"GIF89arest").unwrap(), "gif");
        assert_eq!(detect_extension(b"RIFFxxxxWEBPrest").unwrap(), "webp");
    }

    #[test]
    fn rejects_invalid_background_image_paths() {
        assert!(filename_from_client_path("/tmp/background-x.png").is_err());
        assert!(filename_from_client_path("/api/v1/background-images/../secret.png").is_err());
        assert!(filename_from_client_path("/api/v1/background-images/background-x.svg").is_err());
    }

    #[test]
    fn writes_and_deletes_background_images_inside_owned_directory() {
        let dir = std::env::temp_dir().join(format!("pw-background-{}", new_id()));
        let imported = write_background_image(&dir, b"\x89PNG\r\n\x1a\ncontent").unwrap();
        let filename = imported.filename.clone();
        assert!(dir.join(&filename).is_file());
        delete_background_image_file(&dir, &filename).unwrap();
