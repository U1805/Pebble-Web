...
pub async fn handle_command(
    ...
    match command.as_str() {
        ...
        "health_check" => Ok(Json(health_check_command(state).await?)),
        _ => {
...
async fn health_check_command(state: AppStateRef) -> Result<Value, ApiError> {
...
async fn dispatch(state: AppStateRef, command: &str, args: Value) -> Result<Json<Value>, ApiError> {
    match command {
        ...
        "get_oauth_account_proxy" => Ok(Json(
            oauth_proxy::get_oauth_account_proxy(state, args).await?,
        )),
        "get_oauth_account_proxy_setting" => Ok(Json(
            oauth_proxy::get_oauth_account_proxy_setting(state, args).await?,
        )),
        "update_oauth_account_proxy" => Ok(Json(
            oauth_proxy::update_oauth_account_proxy(state, args).await?,
        )),
        "update_oauth_account_proxy_setting" => Ok(Json(
            oauth_proxy::update_oauth_account_proxy_setting(state, args).await?,
        )),
        "export_backup_file" => Ok(Json(backup::export_backup_file(state, args).await?)),
        "preview_backup_file" => Ok(Json(backup::preview_backup_file(state, args).await?)),
        "import_backup_file" => Ok(Json(backup::import_backup_file(state, args).await?)),
        // WebDAV 云端同步（Web 后端真实实现，能力对齐桌面端）
        "test_webdav_connection" => Ok(Json(webdav::test_webdav_connection(state, args).await?)),
        "backup_to_webdav" => Ok(Json(webdav::backup_to_webdav(state, args).await?)),
        "preview_webdav_backup" => Ok(Json(webdav::preview_webdav_backup(state, args).await?)),
        "restore_from_webdav" => Ok(Json(webdav::restore_from_webdav(state, args).await?)),
        "save_auto_backup_config" => Ok(Json(webdav::save_auto_backup_config(state, args)?)),
        "load_auto_backup_config" => Ok(Json(webdav::load_auto_backup_config(state, args)?)),
        "delete_auto_backup_config" => Ok(Json(webdav::delete_auto_backup_config(state, args)?)),
        ...
        "get_folder_unread_counts" => {
            Ok(Json(folders::get_folder_unread_counts(state, args).await?))
        }
        ...
        "test_imap_connection" => Ok(Json(connection::test_imap_connection(state, args).await?)),
        "test_pop3_connection" => Ok(Json(connection::test_pop3_connection(state, args).await?)),
        "test_account_connection" => Ok(Json(
            connection::test_account_connection(state, args).await?,
        )),
        ...
        "check_for_update" => Ok(Json(update::check_for_update(state, args).await?)),
        // 消息生命周期（本地提交 + 远端排队）
        "archive_message"
        | "restore_message"
        | "delete_message"
        ...
        | "batch_mark_read"
        | "batch_star" => Ok(Json(
            lifecycle::dispatch_command(state, command, args).await?,
        )),
        // 同步（4.6）与待处理队列
