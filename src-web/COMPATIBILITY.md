...
文档导航：[Web 模块说明](README.md) · [开发与维护指南](DEVELOPMENT.md)。
...
多用户、用户管理页、Web Push、Web 专属安全审计页和部署管理页已从当前产品范围移除。

移动端布局和可选 PWA 属于后续 Web 专属工作；PWA 不包含 Web Push。实现顺序和边界见[开发指南](DEVELOPMENT.md#后续移动端与-pwa-边界)。

Docker、Compose、健康检查、备份和升级说明属于部署工程，当前已有相关配置，不属于产品管理页面。这些功能不能用于判断当前 Tauri/Web 业务语义对齐是否完成。
